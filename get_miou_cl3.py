...
if __name__ == "__main__":
    ...
    image_ids = open(os.path.join(
        VOCdevkit_path, "ImageSets/Segmentation/val2.txt"), 'r').read().splitlines()
    gt_dir = os.path.join(VOCdevkit_path, "Labels/")
    time_str = datetime.datetime.strftime(
        datetime.datetime.now(), '%Y_%m_%d_%H_%M_%S')
    # miou_out_path = "cl3miou_mffnetv4_rgb(35)"
    ...
    if miou_mode == 0 or miou_mode == 1:
        ...
        for image_id in tqdm(image_ids):
            image_path = os.path.join(
                VOCdevkit_path, "Images/"+image_id+".tif")
