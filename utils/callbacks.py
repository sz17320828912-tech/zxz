...
class LossHistory():
    ...
    def loss_plot(self):
        ...
        if self.val_loss_flag:
            plt.plot(iters, self.val_loss, 'coral',
        ...
        if epoch % self.period == 0 and self.eval_flag:
            ...
            for image_id in tqdm(self.image_ids):
                ...
                image_path = os.path.join(
                    self.dataset_path, "Images/"+image_id+".tif")
                # 修改1：从gdal读取
                ...
                # 修改3：调整通道顺序
                image = dataset.ReadAsArray(0, 0, self.width, self.height)
                image = image.transpose((1, 2, 0))
                # print("callbacks-image", image.shape)
            ...
            _, IoUs, _, _ = compute_mIoU(
                gt_dir, pred_dir, self.image_ids, self.num_classes, None)  # 执行计算mIoU的函数
            ...
            plt.figure()
            plt.plot(self.epoches, self.mious, 'red',
