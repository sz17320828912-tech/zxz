...
# from nets.msnet import MSNet

# from nets.hed_unet import HEDUNet
...
# from nets.CMFFNet import CMFFNet0
...
# from nets.vitcross_seg_modeling import VisionTransformer
# import nets.vit_seg_configs as CONFIGS


from nets.unet_training import get_lr_scheduler, set_optimizer_lr, weights_init

from utils.callbacks import EvalCallback, LossHistory
from utils.dataloader_tif import UnetDataset, unet_dataset_collate
...
#   这段代码是一个 Python 脚本的主程序，主要用于训练一个图像分割模型。
if __name__ == "__main__":
    ...
    Cuda = True
    #----------------------------------------------#
    #   设置随机种子，用于固定随机数生成器的行为，确保每次训练的结果是可重现的
    #   Seed    用于固定随机种子
    #           使得每次独立训练都可以获得一样的结果
    #----------------------------------------------#
    seed            = 11
    ...
    # model_path  = "model_data/imagenet21k_R50_ViT-B_16.npz"
    model_path  = ""
    ...
    #   （三）batch_size的设置（一次性输入到模型中的样本数量）：
    ...
    #                       (当Freeze_Train=False时失效)
    #------------------------------------------------------------------#
    Init_Epoch          = 0  #   即从第几轮开始训练。在这段代码中，初始轮数被设置为0，意味着训练将从第0轮开始
    Freeze_Epoch        = 0   #   参数冻结将持续50个轮次
    Freeze_batch_size   = 8   #   冻结阶段的批量大小被设置为2
    #------------------------------------------------------------------#
    #   解冻阶段训练参数
    ...
    #------------------------------------------------------------------#
    UnFreeze_Epoch      = 70   # 一共是一百轮，前50论是冻结的，后50论是不冻结的
    ...
    #   batch_size 是指每次模型训练时所使用的样本数量

    #   用于控制是否在训练过程中使用 Dice Loss（Dice损失）作为损失函数
    #------------------------------------------------------------------#
    dice_loss       = False
    #------------------------------------------------------------------#
    #   是否使用focal loss来防止正负样本不平衡
    #   用于控制是否在训练过程中使用 Focal Loss（焦点损失）作为损失函数
    #------------------------------------------------------------------#
    focal_loss      = False
    #------------------------------------------------------------------#
    ...
    #------------------------------------------------------#
    #   设置用到的显卡
    #   用于设置模型训练时所用的设备，确保在分布式训练时每个进程都能正确地使用相应的 GPU 设备进行计算
    #------------------------------------------------------#
    ...
    if pretrained:
        ...
        if distributed:
            ...
            #   使用分布式通信同步所有进程，确保所有进程都下载完成
            dist.barrier()
        else:
            #   如果不是分布式训练模式，下载指定模型的预训练权重文件。
            download_weights(backbone)

    #   根据预训练的情况创建 UNet 模型实例，并将其设置为训练模式。
    # model = Unet(num_classes=num_classes, pretrained=pretrained, backbone=backbone).train()
    # model = MSNet(num_classes=num_classes).train()

    # model = HEDUNet(input_channels=7, num_classes=2, base_channels=32,
    #                 conv_block=Convx2, padding_mode='replicate', batch_norm=True,
    #                 squeeze_excitation=True, merging='attention', stack_height=5,
    #                 deep_supervision=True).train()
    ...
    # model = VisionTransformer(config, img_size=[256, 256], num_classes=num_classes, zero_head=False).train()
    # config = CONFIGS.get_r50_b16_config()
    # model = VisionTransformer(config, img_size=256, num_classes=2, zero_head=False, vis=False).train()

    # ___________________________________________________
    # ___________________________________________________

    # model = CMFPNet(num_classes=num_classes).train()
    # model = UNet(n_channels=10, n_classes=2, bilinear=True).train()
    # model = HighResolutionNet(num_classes=2).train()
    # model = DeepLabV3Plus(num_classes=2).train()
    # model = BHENet().train()
    # model = CMFFNet(num_classes=2).train()
    # model = PVNet().train()
    model = TransUnet(img_dim=256,
                      in_channels=10,
                      out_channels=128,
    ...
    # model = UNetFormer(decode_channels=64, dropout=0.1, backbone_name='swsl_resnet18',
    #                    pretrained=False, window_size=8, num_classes=num_classes).train()

    # config_name = 'R50-ViT-B_16'
    # config = CONFIGS[config_name]
    # model = VisionTransformer(config, img_size=256, num_classes=2, zero_head=True, vis=True).train()

    # ___________________________________________________
    # ___________________________________________________

    #   如果不需要使用预训练的权重，执行下面的操作
    if not pretrained:
        #对模型的权重进行初始化
        weights_init(model)
    #   如果指定了模型权重文件的路径，执行下面的操作
    if model_path != '':
        #------------------------------------------------------#
        #   权值文件请看README，百度网盘下载
        #------------------------------------------------------#
        ...
        #    使用torch.load()函数加载指定路径下的预训练权重文件，并根据当前设备的位置进行映射
        pretrained_dict = torch.load(model_path, map_location = device)
        # pretrained_dict = torch.load(model_path)


        #   定义用于记录加载成功和失败的键的列表，以及用于临时存储加载成功的键值对的字典。
        load_key, no_load_key, temp_dict = [], [], {}
        #   遍历预训练权重文件中的键值对。
        #   讲一整套预训练权重，赋值给vgg的每一层
        for k, v in pretrained_dict.items():
            #   如果预训练权重的键在当前模型的状态字典中，并且其形状与当前模型对应键的形状相匹配，
            #   则将该键值对存储到临时字典中，并将键添加到成功加载的键列表中；
            #   否则，将该键添加到未加载的键列表中。
        ...
        #   更新当前模型的状态字典，将成功加载的键值对添加进去。
        model_dict.update(temp_dict)
        #   将更新后的状态字典应用到当前模型中，实现权重的加载。
    ...
    #----------------------#
    #   记录Loss
    #   根据当前进程的local_rank是否为0来创建或者不创建损失历史记录对象
    #----------------------#
    #   检查当前进程的local_rank是否为0，如果是0，则执行下面的操作
    if local_rank == 0:
        #   获取当前时间，并按照指定的格式（年_月_日_时_分_秒）将其转换为字符串形式
        time_str        = datetime.datetime.strftime(datetime.datetime.now(),'%Y_%m_%d_%H_%M_%S')
        #   将保存目录save_dir和当前时间字符串拼接起来，形成损失记录的保存路径
        log_dir         = os.path.join(save_dir, "loss_" + str(time_str))
        #   创建一个损失历史记录对象，该对象用于记录模型训练过程中的损失值，并指定保存路径、模型对象以及输入形状等参数。
        loss_history    = LossHistory(log_dir, model, input_shape=input_shape)
    #   如果当前进程的local_rank不为0，则执行下面的操作
    else:
        #   将损失历史记录对象设置为None，表示在该进程中不需要创建损失历史记录对象
        loss_history    = None
    ...
    with open(os.path.join(VOCdevkit_path, "ImageSets/Segmentation/train2.txt"),"r") as f:
        train_lines = f.readlines()
